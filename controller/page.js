      ...
      include: [
        {
          ...
          include: [
            {
              model: columns,
              include: [{ model: contents, include: [images] }],
            },
          ],
        },
      ],
...
const deletePage = async (req, res) => {
  ...
  try {
    ...
    if (!page)
      return responseHandler.makeResponseError(res, 404, "page not found");
    ...
    let allPages = (await Promise.all(imagePromises)).every(
      (pagesArray) =>
        pagesArray.length === 1 &&
        pagesArray[0]?.toJSON()?.PageImage?.pageId === id
    );
    if (!allPages)
      return responseHandler.makeResponseError(
        res,
